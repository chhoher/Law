...
public class docBorrowDaoImpl extends DaoUtil implements docBorrowDao{
	...
	public List<LDocBorrowList> findBorrowDoc(int type, String caseId, String bankName, 
			...
			String borrowStartDate, String borrowEndDate, String docCode, String borrowUserId) throws Exception{
		...
		String pcaseId = (caseId != null && !caseId.equals("")) ? "'" + caseId + "'":"null", 
				...
				pborrowUserId = (borrowUserId != null && !borrowUserId.equals("")) ? "'" + borrowUserId + "'":"null";
		
		StringBuffer queryString = new StringBuffer("exec SP_findborrowdocs " + type + "," + pcaseId + "," + pbankName + "," + pisInStore + "," + pdebtName + "," + 
				pborrowReason + "," + pdocStatus + "," + pID + "," + pborrowStartDate + "," + pborrowEndDate + "," + pdocCode + "," + pborrowUserId);
	...
	public boolean saveToUpdateBorrowListStatus(String borrowListDocIds, String updateStatus, String cancelReason) throws Exception{
		log.debug("saveToUpdateBorrowListStatus start");
		String[] DocIds = borrowListDocIds.split(",");
		String[] cancelReasonArray = cancelReason.split(",");
		
		boolean flag = false;
		StringBuffer updateString=new StringBuffer("UPDATE L_DOC_BORROW_LIST");
		if (cancelReason.equals("") || cancelReason == null ) {
			...
			for(int i = 0; DocIds.length > i ; i ++) {
				String tempDocIds = DocIds[i].toString();
				String tempcancelreason = cancelReasonArray[i].toString();
				
				String updateBorrowStatus = " SET borrow_status = " + updateStatus + " , cancel_reason = " + tempcancelreason 
						+ " WHERE borrow_doc_id = " + tempDocIds ;

				log.debug("updateString = {}",updateString + updateBorrowStatus);
				this.jdbcTemplate.execute(updateString + updateBorrowStatus);
			}
